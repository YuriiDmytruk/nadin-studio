...
import { SetType, Product } from '@/types/product'
import { updateProductAction, uploadImageAction } from '../actions/products'
import { X, Upload, Plus, Package, Sparkles, Image as ImageIcon, DollarSign, Type, Palette, AlertCircle, CheckCircle2, ArrowLeft } from 'lucide-react'
...
export default function EditProduct({ product, onCancel }: EditProductProps) {
    ...
    const [setType, setSetType] = useState<SetType>(product.setType)
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const result = await updateProductAction(product.id, updatedProduct)
    ...
    const isFormValid = name.trim() && description.trim() && price && parseFloat(price) > 0 && setType && colors.length > 0 && (existingImageUrls.length > 0 || newImages.length > 0)
        ...
        <Card className="p-8 bg-white/80 backdrop-blur-sm border-2 shadow-lg">
            ...
            <div className="flex items-center gap-3 mb-8 pb-4 border-b border-purple-100">
                ...
                <div className="flex-1">
                    ...
                    <p className="text-sm text-slate-600 mt-1">Update the product details</p>
                </div>
                <Button
                    type="button"
                    variant="outline"
                    onClick={onCancel}
                    className="border-2 border-slate-300 hover:bg-slate-50 hover:border-slate-400 font-semibold transition-all"
                >
                ...
                <div className="space-y-3">
                    ...
                    <div className="space-y-4">
                        ...
                        {existingImageUrls.length > 0 && (
                            <div className="space-y-2">
                                <p className="text-xs font-medium text-slate-600">Existing Images</p>
                        ...
                        {newImages.length > 0 && (
                            <div className="space-y-2">
                                <p className="text-xs font-medium text-slate-600">New Images ({newImages.length})</p>
                                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
